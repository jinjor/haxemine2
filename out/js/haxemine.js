...
org.jinjor.haxemine.Controller = function(socket) {
	...
	var fileSelector = new org.jinjor.haxemine.FileSelector(session,function(file) {
	});
...
org.jinjor.haxemine.FileSelector = function(session,onFileSelected) {
	var files = new Hash();
	Lambda.foreach(session.getAllFiles(),function(f) {
		files.set(f.pathFromProjectRoot,f);
		return true;
	});
	this.container = $("<div/>").on("click li",function() {
		var file = files.get($(this).attr("filePath"));
		onFileSelected(file);
	});
};
...
org.jinjor.haxemine.FileSelector.prototype = {
	render: function(session) {
		this.container.html(org.jinjor.haxemine.FileSelector.template.render({ files : session.getAllFiles(), hasCompileError : function(file) {
